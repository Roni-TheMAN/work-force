...
export function OrganizationCard({ organization, selected, onSelect, onOpenDashboard }: OrganizationCardProps) {
    ...
    >
      ...
      >
        <CardContent className="space-y-4">
          <button type="button" aria-pressed={selected} className="w-full space-y-4 text-left" onClick={() => onSelect(organization.id)}>
          ...
          {onOpenDashboard ? (
            <div className="flex justify-end">
              <Button type="button" size="sm" variant={selected ? "default" : "outline"} onClick={() => onOpenDashboard(organization.id)}>
                Dashboard
              </Button>
            </div>
          ) : null}
